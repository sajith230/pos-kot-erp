...
import { BanquetBooking, BanquetHall, BanquetBookingStatus } from '@/types/database';
...
const statusColors: Record<string, string> = {
  ...
  in_progress: 'bg-green-500/10 text-green-700 border-green-200',
  completed: 'bg-muted text-muted-foreground border-border',
  cancelled: 'bg-red-500/10 text-red-700 border-red-200',
};

export default function BanquetBookings() {
  ...
  const { data: bookings = [], isLoading } = useQuery({
    queryKey: ['banquet-bookings', business?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('banquet_bookings')
        .select('*, banquet_halls(*), customers(*)')
  ...
  const { data: halls = [] } = useQuery({
    queryKey: ['halls-available', business?.id],
    queryFn: async () => {
  ...
  const createBooking = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['banquet-bookings'] });
      setDialogOpen(false);
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    createBooking.mutate({
      ...
      guest_count: Number(fd.get('guest_count')) || 50,
      total_amount: Number(fd.get('total_amount')) || 0,
      advance_paid: Number(fd.get('advance_paid')) || 0,
      catering_included: fd.get('catering_included') === 'on',
      special_requirements: fd.get('special_requirements') || null,
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <PartyPopper className="h-12 w-12 mx-auto text-muted-foreground mb-3" />
          <p className="text-muted-foreground">No banquet bookings found.</p>
        </div>
      ) : (
            ...
            <TableBody>
              {filtered.map((b) => (
                <TableRow key={b.id}>
                  ...
                  <TableCell className="hidden md:table-cell">{b.hall?.name || '—'}</TableCell>
                  <TableCell>{format(new Date(b.event_date), 'MMM d, yyyy')}</TableCell>
                  <TableCell className="hidden sm:table-cell">{b.guest_count}</TableCell>
